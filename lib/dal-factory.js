...
var DalFactory = {
    ...
    Mysql:{
        ...
        //用户登出
        UserLogout: function (userId, userSession, callback) {
            var sqlParam = {
                log_user_id: userId,
                log_session: userSession
            };
            var SQL = "UPDATE log_user_login SET log_session='' " +
                "WHERE (log_session='{log_session}') AND (log_user_id='{log_user_id}')";
            MysqlPoolUtils.Query(SQL, sqlParam, function (err, result) {
                if(err){
                    callback( CommonData.NewFailedFormat(CommonData.DalError.MysqlError, err.code) );
                    return;
